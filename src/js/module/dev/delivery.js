...
define(['jquery', 'jquery.mobile', 'backbone', 'utility/globalRouter',
        ...
        'view/dev/deliveryMenu', 'view/dev/deliveryPanel'], function(
        		...
        		loadCarInfoView, carInfoProcedure, 
       		 	loadDepartureView, departureProcedure, 
       		 	loadDeliveryMenuView, loadDeliveryPanelView){
	...
	var DeliveryModule = Backbone.Model.extend({
		initialize:function(){
			...
			globalRouter.route("view/dev/*path", "delivery", function(path){
				var packagePath = "view/dev/" + path;
				console.log("Route to path : " + packagePath);
				require([packagePath], function(viewLoadingProgress){
					viewLoadingProgress.done(function(view){
						view.render();
					});
				});
			});
		...
		execute:function(){
			 ...
			 */
			//1. 確認車次登入頁面已初始化
			loadCarInfoView.done(function(carInfoView){
				//2. 跳轉至車次登入畫面
			...
			}).fail(function(){//若載入車次登入頁面失敗
				//TODO exception handling
				deferred.reject();
			});
			
			/**
			 *	若使用者確認輸入車次與物流地點, 系統會 1. 初始化出車確認畫面, 然後 2. 顯示出車相關資訊, 
			 *	接著等待使用者後續操作 (確認出車或僅登入)
			 */
			self.listenTo(carInfoProcedure, "confirmed", function(){
				
				//1. 初始化出車確認畫面
				loadDepartureView.done(function(departureView){
					//2. 顯示出車相關資訊
					globalRouter.navigate("view/dev/departure", {trigger:true});
					
				}).fail(function(){//若載入出門確認頁面失敗
					//TODO exception handling
				});
			});
				
			/**
			 * 當使用者出車確認或僅登入時, 系統會 1.確認配送側邊選單已初始化 
			 * 2.確認配送功能選單畫面已初始化 3. 顯示配送功能選單畫面, 等待使用者後續操作
			 */
			self.listenTo(departureProcedure, "departure.confirmed departure.loginOnly", function(){
				...
				//1. 確認配送功能側邊選單已初始化
				loadDeliveryPanelView.done(function(deliveryPanel){
					
				});
				
				//2. 確認配送功能選單畫面已初始化
				loadDeliveryMenuView.done(function(deliveryMenuView){
					
					//3. 跳轉至配送功能主選單, 等待使用者操作
